terraform {
  backend "s3" {
    ...
    key            = "dev/lambda/api/national-teams/terraform.tfstate"
    region         = "eu-central-1"
    encrypt        = "true"
...
module "lambda_get_national_team_candidates" {
  ...
  handler = "index.lambda_handler_get_national_team_candidates"
}
